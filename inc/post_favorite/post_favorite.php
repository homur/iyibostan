...
class NectarLove {
	...
	function add_love($unit = '',$show_icon = true) {
		...
		$output = $this->love_post($post->ID);
  		...
  		$class = 'nectar-love';
  		$icon = 'material-icons';
		$icon_text='favorite_border';
  		$title = esc_html__('Favorite', 'wp-organic');
		...
		$heart_icon = '';
		if($show_icon){
			$heart_icon = '<i class="'.$icon.'">'.$icon_text.'</i>';
		}

		return '<a  href="#" class="'. $class .'" id="nectar-love-'. $post->ID .'" data-toggle="tooltip" data-placement="bottom" title="'. $title .'"> '.$heart_icon . $output .' '.$unit.'</a>';
	}
