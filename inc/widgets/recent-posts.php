...
class CS_Recent_Post_Carousel extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
                'cs_recent_post_carousel', esc_html__('CMS Recent Post','wp-organic'), array('description' => esc_html__('Recent Posts Widget.', 'wp-organic'),)
        );
    }
    ...
    function widget($args, $instance) {
        ...
        $title = apply_filters('widget_title', empty($instance['title']) ? esc_html__('Recent Posts', 'wp-organic' ) : $instance['title'], $instance, $this->id_base);
                        ...
                        <?php while ($wp_query->have_posts()): $wp_query->the_post(); ?>
                            <li class="item clearfix">
                                <div class="cms-recent-details <?php if(has_post_thumbnail() == '') {echo 'no-image';} ?>">
                                    ...
                                    <div class="desc"><?php wp_organic_the_excerpt(100, '', esc_html__('...', 'wp-organic'));?></div>
                                </div>
                            </li>
                        <?php endwhile; ?>
                    </ul>
