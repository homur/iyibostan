...
function cms_options($params = array())
{
    ...
    if (is_admin() && !empty($params['id']) && isset($core_options)) {
        ...
        elseif ($pagenow=='post-new.php' || $pagenow=='post.php'){
            ...
            $wp_organic_meta = (array)$wp_organic_meta;
            // Get value
            if(!empty($wp_organic_meta[$params['id']])){
                $params['value'] = $wp_organic_meta[$params['id']];
            } else {
                $params['value'] = null;
            }
...
class CsCoreControl
{
    ...
    function Scripts()
    {
        ...
        wp_register_style('select2-css', get_template_directory_uri() . '/inc/metacore/assets/css/select2.min.css');
        ...
        wp_register_script('select2-js', get_template_directory_uri() . '/inc/metacore/assets/js/select2.full.min.js');
        ...
        if(isset($params['type'])){
            switch ($params['type']){
                ...
                case 'editor':
                    ...
                    break;
                case 'multiple_input':
                    return $this->multiple_input($params);
                    break;
            }
        }
    }
    private function text($params)
    {
        ...
        $attr = isset($params['attributes']) ? $params['attributes'] : array();
        ...
        placeholder="<?php if(isset($params['placeholder'])){ echo esc_attr($params['placeholder']);} ?>" <?php echo implode(' ', $attr); ?>/>
        </div>
        <?php
        return ob_get_clean();
    }
    private function textarea($params)
    {
        ...
        <textarea
            rows="<?php echo isset($params['rows']) ? esc_attr($params['rows']) : 3; ?>"
            name="<?php echo esc_attr($params['id']); ?>"
            id="<?php echo esc_attr($params['id']); ?>"
        ...
        <div class="images-field clearfix">
            ...
            foreach ($selected as $value):
                ...
                <li class="items" data-id="<?php echo esc_attr($value); ?>" style="background-image:url(<?php echo esc_url($attachment_image[0]);?>);background-size:cover;">
                    <i class="edit dashicons dashicons-plus-alt" title="<?php esc_html_e('Replace Image', 'wp-organic'); ?>"></i>
                    <i class="remove dashicons dashicons-dismiss" title="<?php esc_html_e('Remove Image', 'wp-organic'); ?>"></i>
                </li>
            <?php endif; endforeach; ?>
            <?php if($params['field'] != 'single'): ?>
                <li class="items" data-id=""><i class="add dashicons dashicons-plus-alt" title="<?php esc_html_e('Add Image', 'wp-organic'); ?>"></i></li>
    ...
    private function multiple_input($params){
        global $post;
        ...
        ob_start();
        ?>
        <div class="multiple-input-field csfield">
            <?php if(!empty($params['inputs'])):?>
                <?php $__i = 0; foreach ($params['inputs'] as $_input): ?>
                    <div class="mi-fields">
                        <?php foreach ($_input as $_in) {
                            if (isset($value[$_in['id']][$__i])) $_in['value'] = $value[$_in['id']][$__i];
                            $_in['id'] = $id . '[' . $_in['id'] . '][]';
                            $this->metabox($_in);;
                        }
                        $__i++; ?>
                    <button type="button" class="mi-remove button"><?php esc_html_e('REMOVE', 'wp-organic'); ?></button>
                </div>
                <?php endforeach; ?>
            <?php endif; ?>
            <button type="button" class="mi-add button button-primary button-large"><?php esc_html_e('ADD', 'wp-organic'); ?></button>
        </div>
        <?php
        return ob_get_clean();
    }
    ...
    public function save_meta_boxes($post_id)
    {
        ...
        /* array cms meta */
        $wp_organic_meta = array();

        /* find cms meta key. */
        foreach($_POST as $key => $value) {
            if(strstr($key, '_cms_')) {
                $wp_organic_meta[$key] = rawurlencode($value);
                update_post_meta($post_id, $key, $value);
            }
        }
        /* update _cms_meta_data. */
        if(!empty($wp_organic_meta)){
          update_post_meta($post_id, '_cms_meta_data', json_encode($wp_organic_meta));
        }
    }
}