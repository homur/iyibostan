jQuery(document).ready(function($) {
	 ...
	 * @private
     */
	var _ingredient = $('#cs_metabox_field__cms_ingredients').find('input[name="_cms_ingredients[ingredient][]"]');
	_ingredient.each(function () {

		//wp_select2($(this));

		if($(this).val() != '') {
			//$(this).select2('data', {id: $(this).val(), text: $(this).data('initvalue')});
		}
	});

	$('.multiple-input-field').on('click', '.mi-remove', function () {
		$(this).parents('.mi-fields').remove();
	});

	$('.multiple-input-field').on('click', '.mi-add', function () {
		var _multiple = $(this).parents('.multiple-input-field');
		var _field = _multiple.find('.mi-fields').last().clone();
			_field.find('.select2-container').remove();
			_field.prependTo(_multiple);
		//wp_select2(_field.find('input[name="_cms_ingredients[ingredient][]"]'));
	});

	function wp_select2(_field) {
		_field.select2({
			ajax:{
				url: ajaxurl,
				dataType: 'json',
				...
				data:function (params) {
					return {
						q: params,
						action: 'wp_organic_product_search',
					};
				},
